/**
 * Cozumleyici cekirdegi — bir sayfadaki medya adaylarini bulur.
 *
 ...
 *
 * Fonksiyon degil kutuphanedir: hem /api/resolve (tarayici arayuzu) hem de
 * /api/shortcut (iPhone Kisayollar) ayni cozumlemeyi kullanir.
 */
...
export type Kind = "hls" | "dash" | "pair" | "video" | "audio";
...
/** Cozumleme sonucu — /api/resolve yanitinin govdesi ile ayni. */
...
/**
 * Verilen sayfayi cozumler ve bulunan medya adaylarini dondurur.
 *
 * `pageUrl` cagirandan guvenli oldugu dogrulanmis (assertSafeUrl'den gecmis)
 * olarak gelir. Sayfaya hic ulasilamadiginda HttpError firlatir.
 */
export async function resolvePage(pageUrl: URL, cookie?: string): Promise<ResolveResult> {
  ...
  if (!helper.authoritative) {
    try {
      ...
      if (out.size === 0) {
        throw new HttpError(502, `Sayfa okunamadi: ${(err as Error).message}`);
      }
    }
